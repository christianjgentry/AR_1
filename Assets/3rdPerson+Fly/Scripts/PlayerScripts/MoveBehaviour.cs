...
public class MoveBehaviour : GenericBehaviour
{
	...
	void Update ()
	{
		...
		if (!jump && Input.GetButtonDown(jumpButton) && behaviourManager.IsCurrentBehaviour(this.behaviourCode) && !behaviourManager.IsOverriding())
		{
            if (!behaviourManager.IsHorizontalMoving())
            {
                ...
                behaviourManager.GetRigidBody.AddForce(transform.up * velocity, ForceMode.VelocityChange);
            }
            else
            {
                jump = true;
        ...
        if (Input.GetButtonDown(jumpButton) && canDoublejump && !behaviourManager.IsGrounded())
        {
            if (behaviourManager.GetRigidBody.velocity.y > -3f)
            {
                float velocity =  2f * Mathf.Abs(Physics.gravity.y) * jumpHeight;
                velocity = Mathf.Sqrt(velocity);
	...
	void MovementManagement(float horizontal, float vertical)
	{
		...
		{
			speed = sprintSpeed;
            jumpIntertialForce = 17;
            jumpHeight = 1.3f;
		}
        else
        {
            jumpIntertialForce = 10;
            jumpHeight = 1.3f;
        }
